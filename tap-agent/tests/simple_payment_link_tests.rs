...
async fn test_basic_oob_creation_and_parsing() {
    ...
    assert_eq!(attachment.media_type.as_deref(), Some("application/didcomm-signed+json"));
}
...
async fn test_url_round_trip() {
    ...
    let oob_invitation = OutOfBandInvitation::from_url(&original_url)
        .expect("Failed to parse OOB from URL");
...
fn test_oob_validation() {
    ...
    let valid_oob = OutOfBandInvitation::builder(
        "did:example:test",
        "tap.payment",
        "Valid payment"
    ).build();
    ...
    let invalid_goal_oob = OutOfBandInvitation::builder(
        "did:example:test",
        "invalid.goal",
        "Invalid goal"
    ).build();
    ...
    let mut no_didcomm_oob = OutOfBandInvitation::builder(
        "did:example:test",
        "tap.payment",
        "No DIDComm"
    ).build();
