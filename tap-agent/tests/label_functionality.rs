...
fn test_find_by_label() -> Result<()> {
    ...
    let key1 = generator.generate_ed25519_did()?;
    storage.add_key(KeyStorage::from_generated_key_with_label(&key1, "signing-key"));
    
    let key2 = generator.generate_p256_did()?;
    storage.add_key(KeyStorage::from_generated_key_with_label(&key2, "encryption-key"));
...
fn test_storage_persistence_with_labels() -> Result<()> {
    ...
    // Create and save storage with labeled keys
    {
        ...
        let key1 = generator.generate_ed25519_did()?;
        storage.add_key(KeyStorage::from_generated_key_with_label(&key1, "test-key-1"));
        
        let key2 = generator.generate_p256_did()?;
        storage.add_key(KeyStorage::from_generated_key_with_label(&key2, "test-key-2"));
...
fn test_mixed_labeled_and_unlabeled_keys() -> Result<()> {
    ...
    // Add labeled key
    let key1 = generator.generate_ed25519_did()?;
    storage.add_key(KeyStorage::from_generated_key_with_label(&key1, "custom-label"));
    ...
    storage.add_key(KeyStorage::from_generated_key_with_label(&key3, "another-custom"));
