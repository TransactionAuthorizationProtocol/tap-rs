use tap_agent::{
    ...
    agent_key::{AgentKey, SigningKey, VerificationKey, EncryptionKey, DecryptionKey, JwsAlgorithm},
    local_agent_key::LocalAgentKey,
    message_packing::{Packable, Unpackable, PackOptions, UnpackOptions, PlainMessage},
    error::Result,
};
...
async fn test_key_manager_key_operations() -> Result<()> {
    ...
    // Test verification
    assert!(key_manager.verify("ed25519-key", test_data, &signature).await.is_ok());
