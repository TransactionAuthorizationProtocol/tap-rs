use tap_agent::{
    agent::{Agent, ModernAgent, ModernAgentBuilder, DeliveryResult, AgentConfig},
    key_manager::{DefaultKeyManager, KeyManagerBuilder},
    message_packing::{Packable, Unpackable, PackOptions, UnpackOptions},
    error::{Result, Error},
};
...
async fn test_modern_agent_creation() -> Result<()> {
    ...
    assert_eq!(agent.config().service_endpoint, "https://example.com/endpoint");
