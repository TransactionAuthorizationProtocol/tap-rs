...
use tap_msg::{Authorize, Invoice, LineItem, PaymentRequest, Participant, Settle, TaxCategory, TaxSubtotal, TaxTotal};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let invoice_id = format!("INV-{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap());
    
    // Create a payment message with an invoice
    let payment = create_payment_message_with_invoice(&merchant_did, &customer_did, settlement_address, &invoice_id);
    ...
    if let Some(invoice) = &payment.invoice {
        ...
        println!("  Invoice Total: {} {}", invoice.total, invoice.currency_code);
...
fn create_payment_message_with_invoice(
    ...
    let invoice = Invoice {
        ...
        due_date: Some((chrono::Utc::now() + chrono::Duration::days(30)).date_naive().to_string()),
