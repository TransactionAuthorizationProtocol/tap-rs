...
async fn main() -> Result<()> {
    ...
    let transfer = match create_transfer_message(&originator_did, &beneficiary_did, settlement_address) {
        Ok(transfer) => transfer,
        Err(e) => {
            println!("Error creating transfer message: {}", e);
            return Err(Error::Validation("Failed to create transfer message".to_string()));
        }
    };
    ...
    if let Err(e) = transfer.validate() {
        ...
        return Err(Error::Validation(format!("Transfer validation failed: {}", e)));
    }
    ...
    let packed_transfer = match originator_agent.send_message(&transfer, &beneficiary_did).await {
        Ok(packed) => packed,
        Err(e) => {
    ...
    let _received_transfer: Transfer = match beneficiary_agent.receive_message(&packed_transfer).await {
        Ok(transfer) => transfer,
        Err(e) => {
    ...
    println!("  Risk score: {}/100 (threshold: {})\n", risk_score, risk_threshold);
    ...
    if risk_score >= risk_threshold {
        ...
        let received_reject: Reject = match originator_agent.receive_message(&packed_reject).await {
            Ok(reject) => reject,
            Err(e) => {
    ...
    let received_authorize: Authorize = match originator_agent.receive_message(&packed_authorize).await {
