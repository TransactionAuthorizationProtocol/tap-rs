...
use crate::did::{DIDGenerationOptions, DIDKeyGenerator, GeneratedKey, KeyType, MultiResolver, SyncDIDResolver};
...
fn lookup_did(did: &str, output: Option<PathBuf>) -> Result<()> {
    ...
    let rt = Runtime::new().map_err(|e| Error::DIDResolution(format!("Failed to create runtime: {}", e)))?;
