...
use tap_wasm::{MessageType, WasmTapAgent, TapNode, generate_uuid_v4};
...
fn test_wasm_tap_agent_creation() {
    ...
    js_sys::Reflect::set(&config, &JsValue::from_str("debug"), &JsValue::from_bool(true)).unwrap();
    js_sys::Reflect::set(&config, &JsValue::from_str("nickname"), &JsValue::from_str("test-agent")).unwrap();
...
fn test_tap_node_creation() {
    let config = Object::new();
    js_sys::Reflect::set(&config, &JsValue::from_str("debug"), &JsValue::from_bool(false)).unwrap();
...
fn test_agent_message_creation() {
    ...
    assert_eq!(message_type.as_string().unwrap(), "https://tap.rsvp/schema/1.0#Transfer");
