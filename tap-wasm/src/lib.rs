...
impl Message {
    ...
    pub fn set_authorization_request(&mut self, private_payload: JsValue) {
        let json_value: serde_json::Value = serde_wasm_bindgen::from_value(private_payload).unwrap_or(serde_json::Value::Null);
        self.auth_request = Some(AuthorizationRequest { private_payload: json_value });
    }

    /// Sets the authorization response data
    pub fn set_authorization_response(&mut self, private_payload: JsValue, signed_date: String, valid_until: Option<String>) {
        let json_value: serde_json::Value = serde_wasm_bindgen::from_value(private_payload).unwrap_or(serde_json::Value::Null);
    ...
    pub fn authorization_request(&self) -> JsValue {
        match &self.auth_request {
            Some(request) => {
                ...
                let payload_js = serde_wasm_bindgen::to_value(&request.private_payload).unwrap_or(JsValue::NULL);
    ...
    pub fn authorization_response(&self) -> JsValue {
        match &self.auth_response {
            Some(response) => {
                let obj = js_sys::Object::new();
                let payload_js = serde_wasm_bindgen::to_value(&response.private_payload).unwrap_or(JsValue::NULL);
                Reflect::set(&obj, &JsValue::from_str("privatePayload"), &payload_js).unwrap();
                Reflect::set(&obj, &JsValue::from_str("signedDate"), &JsValue::from_str(&response.signed_date)).unwrap();
                if let Some(valid_until) = &response.valid_until {
                    Reflect::set(&obj, &JsValue::from_str("validUntil"), &JsValue::from_str(valid_until)).unwrap();
                }
                obj.into()
            },
            None => JsValue::NULL,
    ...
    pub fn to_bytes(&self) -> Result<js_sys::Uint8Array, JsValue> {
        match serde_json::to_string(self) {
            ...
            Err(err) => Err(JsValue::from_str(&format!("Failed to serialize message: {}", err))),
        }
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Message, JsValue> {
        match std::str::from_utf8(bytes) {
            Ok(json) => match serde_json::from_str(json) {
                ...
                Err(err) => Err(JsValue::from_str(&format!("Failed to deserialize message: {}", err))),
            },
            Err(err) => Err(JsValue::from_str(&format!("Failed to convert bytes to UTF-8: {}", err))),
        }
    }
...
impl Agent {
    ...
    pub fn new(config: JsValue) -> Self {
        let did = if let Ok(did_prop) = Reflect::get(&config, &JsValue::from_str("did")) {
            if let Some(did_str) = did_prop.as_string() {
                ...
                format!("did:key:z6Mk{}", uuid::Uuid::new_v4().to_string().replace("-", ""))
            }
        } else {
            // Generate a random DID as fallback
            format!("did:key:z6Mk{}", uuid::Uuid::new_v4().to_string().replace("-", ""))
    ...
    pub fn process_message(&self, message: JsValue, metadata: JsValue) -> Promise {
        ...
        future_to_promise(async move {
            ...
            let parsed_message: Result<Message, serde_wasm_bindgen::Error> = serde_wasm_bindgen::from_value(message_clone.clone());
            ...
            if let Ok(msg) = parsed_message {
                ...
                if let Some(handler) = this.message_handlers.get(&msg.message_type) {
                    ...
                    args.set(0, serde_wasm_bindgen::to_value(&msg).unwrap_or(message_clone.clone()));
                ...
                if let Ok(message_type) = Reflect::get(&message_clone, &JsValue::from_str("message_type")) {
...
impl TapNode {
    ...
    fn try_parse_message_struct(message_clone: &JsValue, debug: bool) -> Result<Option<Message>, JsValue> {
        // Try to parse the message to see if it's a Message struct
        let message_struct: Result<Message, _> = serde_wasm_bindgen::from_value(message_clone.clone());
        ...
        match &message_struct {
            ...
            Err(err) => {
                if debug {
                    console::warn_1(&JsValue::from_str(
                        &format!("Failed to parse message as Message struct: {:?}", err)
                    ));
                }
                Ok(None)
            }
    ...
    pub fn process_message(&self, message: JsValue, metadata: JsValue) -> Promise {
        ...
        future_to_promise(async move {
            ...
            let metadata_map: Result<HashMap<String, String>, _> = serde_wasm_bindgen::from_value(metadata_clone.clone());
            ...
            if let Ok(metadata_map) = metadata_map {
                ...
                if let Some(to_did) = metadata_map.get("toDid") {
                    if let Some(agent) = this.agents.get(to_did) {
                        ...
                        console::warn_1(&JsValue::from_str(
                            &format!("Agent with DID {} not found", to_did)
                        ));
                    } else {
