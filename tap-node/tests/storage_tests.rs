#[cfg(feature = "storage")]
mod storage_tests {
    use tap_node::storage::{Storage, StorageError, MessageDirection, TransactionType, TransactionStatus};
    use tap_msg::didcomm::PlainMessage;
    use tap_msg::message::{transfer::Transfer, payment::Payment, Participant};
    use serde_json::json;
    ...
    fn create_payment_message(id: &str) -> PlainMessage {
        let payment_body = Payment {
            transaction_id: id.to_string(),
            asset: Some("eip155:1/erc20:0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48".parse().unwrap()),
    ...
    async fn test_in_memory_storage_creation() {
        ...
        let msg = create_test_message("test_1", "https://tap-protocol.io/messages/connect/1.0", json!({}));
        assert!(storage.log_message(&msg, MessageDirection::Incoming).await.is_ok());
    }
    ...
    async fn test_message_logging_all_directions() {
        ...
        let incoming1 = create_test_message("in_001", "https://tap-protocol.io/messages/connect/1.0", json!({}));
        let incoming2 = create_test_message("in_002", "https://tap-protocol.io/messages/authorize/1.0", json!({}));
        
        storage.log_message(&incoming1, MessageDirection::Incoming).await.unwrap();
        storage.log_message(&incoming2, MessageDirection::Incoming).await.unwrap();
        
        // Log outgoing messages
        let outgoing1 = create_test_message("out_001", "https://tap-protocol.io/messages/reject/1.0", json!({}));
        let outgoing2 = create_test_message("out_002", "https://tap-protocol.io/messages/settle/1.0", json!({}));
        
        storage.log_message(&outgoing1, MessageDirection::Outgoing).await.unwrap();
        storage.log_message(&outgoing2, MessageDirection::Outgoing).await.unwrap();
        ...
        let incoming_only = storage.list_messages(10, 0, Some(MessageDirection::Incoming)).await.unwrap();
        assert_eq!(incoming_only.len(), 2);
        assert!(incoming_only.iter().all(|m| m.direction == MessageDirection::Incoming));
        
        let outgoing_only = storage.list_messages(10, 0, Some(MessageDirection::Outgoing)).await.unwrap();
        assert_eq!(outgoing_only.len(), 2);
        assert!(outgoing_only.iter().all(|m| m.direction == MessageDirection::Outgoing));
    }
    ...
    async fn test_message_retrieval_by_id() {
        ...
        storage.log_message(&msg, MessageDirection::Incoming).await.unwrap();
        ...
        assert_eq!(stored_msg.message_type, "https://tap-protocol.io/messages/transfer/1.0");
    ...
    async fn test_message_duplicate_handling() {
        ...
        let msg = create_test_message("dup_msg_001", "https://tap-protocol.io/messages/connect/1.0", json!({}));
    ...
    async fn test_message_thread_tracking() {
        ...
        let mut parent_msg = create_test_message("parent_001", "https://tap-protocol.io/messages/transfer/1.0", json!({}));
        parent_msg.thid = Some("thread_parent".to_string());
        parent_msg.pthid = None;
        
        let mut child_msg = create_test_message("child_001", "https://tap-protocol.io/messages/authorize/1.0", json!({}));
        child_msg.thid = Some("thread_child".to_string());
        child_msg.pthid = Some("thread_parent".to_string());
        
        storage.log_message(&parent_msg, MessageDirection::Incoming).await.unwrap();
        storage.log_message(&child_msg, MessageDirection::Outgoing).await.unwrap();
        
        // Retrieve and verify thread relationships
        let parent = storage.get_message_by_id("parent_001").await.unwrap().unwrap();
        assert_eq!(parent.thread_id, Some("thread_parent".to_string()));
        assert_eq!(parent.parent_thread_id, None);
        
        let child = storage.get_message_by_id("child_001").await.unwrap().unwrap();
    ...
    async fn test_non_transaction_messages_not_in_transactions_table() {
        ...
        let connect_msg = create_test_message("connect_001", "https://tap-protocol.io/messages/connect/1.0", json!({}));
        let auth_msg = create_test_message("auth_001", "https://tap-protocol.io/messages/authorize/1.0", json!({}));
        
        // Log them as messages
        storage.log_message(&connect_msg, MessageDirection::Incoming).await.unwrap();
        storage.log_message(&auth_msg, MessageDirection::Incoming).await.unwrap();
        ...
        let transfer_msg = create_transfer_message("transfer_001");
        storage.log_message(&transfer_msg, MessageDirection::Incoming).await.unwrap();
        storage.insert_transaction(&transfer_msg).await.unwrap();
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                storage_clone.log_message(&msg, MessageDirection::Incoming).await
            });
            handles.push(handle);
        }
        ...
        assert!(successes >= 8, "Only {} out of 10 concurrent operations succeeded", successes);
        
        // Verify messages were inserted
        let messages = storage.list_messages(20, 0, None).await.unwrap();
        assert!(messages.len() >= 8, "Only {} messages were inserted", messages.len());
    }
    ...
    async fn test_storage_error_handling() {
        ...
        assert!(matches!(result, Err(StorageError::InvalidTransactionType(_))));
    ...
    async fn test_message_json_integrity() {
        ...
        let msg = create_test_message("json_test_001", "https://tap-protocol.io/messages/test/1.0", complex_body);
        
        storage.log_message(&msg, MessageDirection::Incoming).await.unwrap();
        
        // Retrieve and verify JSON integrity
        let retrieved = storage.get_message_by_id("json_test_001").await.unwrap().unwrap();
