...
use tap_node::{NodeConfig, Result, TapNode};
use uuid;
...
async fn test_message_routing() -> Result<()> {
    ...
    let packed_message = node
        .send_message(agent1_did, agent2_did, message.clone())
        .await?;
    ...
    let _event = tokio::time::timeout(std::time::Duration::from_secs(1), event_receiver.recv())
        .await
