...
impl TapNode {
    ...
    /// Receive and process an encrypted/signed message
    ///
    /// This method handles encrypted or signed DIDComm messages by:
    /// 1. Determining the recipient agent based on the encrypted message metadata
    /// 2. Forwarding the message to the appropriate agent for decryption
    /// 3. Processing the decrypted message through the normal pipeline
    ///
    /// # Parameters
    ///
    /// * `encrypted_message` - The encrypted/signed DIDComm message as a string
    ///
    /// # Returns
    ///
    /// * `Ok(())` if the message was successfully processed
    /// * `Err(Error)` if there was an error during decryption or processing
    pub async fn receive_encrypted_message(&self, encrypted_message: String) -> Result<()> {
        use tap_agent::{SecurityMode, UnpackOptions, Unpackable};

        // Parse the encrypted message to extract recipient information
        let json_value: serde_json::Value =
            serde_json::from_str(&encrypted_message).map_err(|e| Error::Serialization(e))?;

        // For encrypted messages, we need to extract the recipient from the message
        // The recipient information might be in the "recipients" field for authcrypt
        // or we might need to try all registered agents for signed messages

        // First, check if there's a "recipients" field (authcrypt)
        if let Some(recipients) = json_value.get("recipients").and_then(|r| r.as_array()) {
            // Try to find an agent that can decrypt this message
            for recipient in recipients {
                if let Some(header) = recipient.get("header").and_then(|h| h.as_object()) {
                    if let Some(kid) = header.get("kid").and_then(|k| k.as_str()) {
                        // Extract DID from kid (format: "did:example:123#keys-1")
                        if let Some(did) = kid.split('#').next() {
                            // Try to get the agent for this DID
                            match self.agents.get_agent(did).await {
                                Ok(agent) => {
                                    // Create unpack options
                                    let unpack_options = UnpackOptions {
                                        expected_security_mode: SecurityMode::Any,
                                        expected_recipient_kid: Some(kid.to_string()),
                                        require_signature: false,
                                    };

                                    // Try to unpack with this agent's key manager
                                    let agent_ref: &TapAgent = &*agent;
                                    match String::unpack(
                                        &encrypted_message,
                                        &**agent_ref.key_manager(),
                                        unpack_options,
                                    )
                                    .await
                                    {
                                        Ok(plain_message) => {
                                            log::info!(
                                                "Successfully decrypted message with agent {}",
                                                did
                                            );
                                            return self.receive_message(plain_message).await;
                                        }
                                        Err(e) => {
                                            log::debug!(
                                                "Agent {} couldn't decrypt message: {}",
                                                did,
                                                e
                                            );
                                            continue;
        ...
        // If no recipients field or none of the recipients worked, try all agents
        // This handles signed messages or cases where we couldn't determine the recipient
        let all_dids = self.agents.get_all_dids();
        for did in all_dids {
            match self.agents.get_agent(&did).await {
                Ok(agent) => {
                    // Create unpack options
                    let unpack_options = UnpackOptions {
                        expected_security_mode: SecurityMode::Any,
                        expected_recipient_kid: Some(format!("{}#keys-1", did)),
                        require_signature: false,
                    };

                    // Try to unpack with this agent
                    let agent_ref: &TapAgent = &*agent;
                    match String::unpack(
                        &encrypted_message,
                        &**agent_ref.key_manager(),
                    ...
                    {
                        Ok(plain_message) => {
                            log::info!("Successfully decrypted message with agent {}", did);
                            return self.receive_message(plain_message).await;
                        }
                        Err(_) => continue,
                    }
        ...
        Err(Error::MessageDropped(
            "No agent could decrypt the message".to_string(),
        ))
    }
