...
describe('Transfer', () => {
  ...
  it('should set all required fields in the body', () => {
    expect(transfer.asset).toBe('eip155:1/erc20:0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48');
    expect(transfer.amount).toBe('100.50');
    expect(transfer.originator).toEqual(mockOriginator);
    expect(transfer.beneficiary).toEqual(mockBeneficiary);
    expect(transfer.agents).toContainEqual(mockAgent);
  });
  ...
  it('should set optional fields when provided', () => {
    ...
    expect(transferWithOptions.settlementId).toBe('eip155:1/tx/0x123abc');
    expect(transferWithOptions.memo).toBe('Test transfer');
    expect(transferWithOptions.purpose).toBe('CASH');
    expect(transferWithOptions.categoryPurpose).toBe('CASH');
    expect(transferWithOptions.expiry).toBe('2023-12-31T23:59:59Z');
  });
  ...
  it('should validate required fields', () => {
    ...
    // Test missing asset
    const tempAsset = transfer.asset;
    (transfer as any).asset = undefined;
    expect(() => transfer._validate()).toThrow(ValidationError);
    transfer.asset = tempAsset;

    // Test missing amount
    const tempAmount = transfer.amount;
    (transfer as any).amount = undefined;
    expect(() => transfer._validate()).toThrow(ValidationError);
    transfer.amount = tempAmount;

    // Test missing originator
    const tempOriginator = transfer.originator;
    (transfer as any).originator = undefined;
    expect(() => transfer._validate()).toThrow(ValidationError);
    transfer.originator = tempOriginator;

    // Test missing agents
    const tempAgents = transfer.agents;
    (transfer as any).agents = undefined;
    expect(() => transfer._validate()).toThrow(ValidationError);
    (transfer as any).agents = [];
    expect(() => transfer._validate()).toThrow(ValidationError);
    transfer.agents = tempAgents;
  });
