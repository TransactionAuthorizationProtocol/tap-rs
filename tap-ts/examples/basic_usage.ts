...
function createSigner(privateKey: string, did: string) {
  return {
    sign: async (data: Uint8Array): Promise<Uint8Array> => {
      console.log(`[Signing ${data.length} bytes with key ${privateKey.slice(0, 8)}...]`);
...
async function main() {
  try {
    ...
    const originatorDid = 'did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK';
    const beneficiaryDid = 'did:key:z6MkgYAGxLBSJEm1JAHFuGVK7nzHBRXSkmGRRiZNqvz1N9GK';
    ...
    console.log(`Originator agent created with DID: ${originatorAgent.getDID()}`);
    console.log(`Beneficiary agent created with DID: ${beneficiaryAgent.getDID()}`);
    ...
    console.log(`Transfer verification result: ${isValid ? 'Valid' : 'Invalid'}`);
    ...
    if (isValid) {
      ...
      const settlementTxId = 'eip155:1/tx/0x4a563af33c4871b51a8b108aa2fe1dd5280a30dfb7236170ae5e5e7957eb6392';
      
      const settle = receivedTransfer.settle(settlementTxId, transfer.body.amount);
