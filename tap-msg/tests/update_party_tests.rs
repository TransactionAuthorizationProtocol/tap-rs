...
fn test_update_party_creation() {
    ...
    assert_eq!(update_party.context, Some("https://tap.rsvp/schema/1.0".to_string()));
    ...
    assert_eq!(update_party_with_note.note, Some("Updating party information".to_string()));
}
...
fn test_update_party_didcomm_conversion() {
    ...
    let didcomm_message = update_party.to_didcomm().expect("Failed to convert UpdateParty to DIDComm");
...
fn test_authorizable_with_update_party() {
    ...
    assert_eq!(update_party.party.id, updated_participant.id);
    assert_eq!(update_party.party.role, updated_participant.role);
    assert_eq!(update_party.note, Some("Updated via Authorizable trait".to_string()));
    
    // Now test the same with a DIDComm message
    let message = transfer.to_didcomm().expect("Failed to convert to DIDComm message");
    ...
    assert_eq!(update_party_from_message.party.role, updated_participant.role);
    assert_eq!(update_party_from_message.note, Some("Updated via DIDComm message".to_string()));
}
