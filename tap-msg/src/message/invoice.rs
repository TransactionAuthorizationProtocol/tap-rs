...
pub struct Invoice {
    ...
    #[serde(rename = "additionalDocumentReference", skip_serializing_if = "Option::is_none")]
...
impl Invoice {
    ...
    pub fn validate(&self) -> crate::error::Result<()> {
        ...
        if self.line_items.is_empty() {
            return Err(Error::Validation("At least one line item is required".to_string()));
        }
        
        // Validate line items
        for (i, item) in self.line_items.iter().enumerate() {
            if item.id.is_empty() {
                return Err(Error::Validation(format!("Line item {} is missing an ID", i)));
            }
            
            if item.description.is_empty() {
                return Err(Error::Validation(format!("Line item {} is missing a description", i)));
            }
        ...
        if let Some(sub_total) = self.sub_total {
            let calculated_sub_total: f64 = self.line_items.iter().map(|item| item.line_total).sum();
        ...
        // Validate total
        let sub_total = self.sub_total.unwrap_or_else(|| self.line_items.iter().map(|item| item.line_total).sum());
        ...
        if let Err(_) = chrono::NaiveDate::parse_from_str(&self.issue_date, "%Y-%m-%d") {
            return Err(Error::Validation(
                format!("Issue date ({}) is not in the required ISO 8601 format (YYYY-MM-DD)", self.issue_date)
            ));
        }
        
        if let Some(due_date) = &self.due_date {
            if let Err(_) = chrono::NaiveDate::parse_from_str(due_date, "%Y-%m-%d") {
                return Err(Error::Validation(
                    format!("Due date ({}) is not in the required ISO 8601 format (YYYY-MM-DD)", due_date)
                ));
            }
        }
